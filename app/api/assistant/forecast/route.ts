  ...
  try {
    ...
    if (parsed && parsed.transactions) {
      const txs = parsed.transactions
        ...
        .filter((t: any) => t.category !== 'transfer');

      const catMap: Record<string, number> = {};
      for (const t of txs) {
        const k = t.category || 'other';
        catMap[k] = (catMap[k] || 0) + Math.abs(Number(t.amount || 0));
      }

      totalExpenses = Object.values(catMap).reduce((s: number, v: number) => s + v, 0);
